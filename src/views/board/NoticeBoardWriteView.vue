<template>
  <div class="write-container">
    ...
    <div class="write-header">
      ...
      <h2 class="header-title">공지사항 쓰기</h2>
...
const onCrop = async (blob) => {
  try {
    ...
    // NOTICE_BOARD type usage if required by backend, using FREE_BOARD for now as similar or ask user?
    // User said "code is mostly same", assuming file upload type might need checking but typically same logic.
    // Using "NOTICE_BOARD" if it makes sense, but `fileApi` might expect specific enum.
    // Checking `fileApi` definition would be best but for now sticking to FREE_BOARD or NOTICE if valid.
...
const removeImage = async (index) => {
  ...
  if (image.id) {
    try {
      await fileApi.deleteFile(image.id, "NOTICE");
    } catch (err) {
      console.error(err);
    }
  }
...
const submitPost = async () => {
  const postData = {
    ...
    imageIds: croppedImages.value.map(img => img.id),
  };

  try {
    const res = await boardApi.postNoticeBoard(postData);
    const newPostId = res.id;
    isSaved.value = true;
    router.replace({ name: 'noticeboard-detail', params: { id: newPostId } });
  } catch (err) {
    console.error(err);
