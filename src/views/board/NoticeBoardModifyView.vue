<template>
  <div class="write-container">
    ...
    <div class="write-header">
      ...
      <h2 class="header-title">공지사항 수정</h2>
...
onMounted(() => {
  if (route.params.id) {
    ...
    boardApi.getNoticeBoardDetail(editId.value).then(async res => {  
      ...
      content.value = data.content || data.title; 
      if (data.noticeImages) {
        data.noticeImages.forEach(img => {
...
const submitPost = async () => {
  ...
  try {
    await boardApi.updateNoticeBoard(editId.value, updateData); 

    updateData.id = editId.value;

    const newPostId = editId.value;
